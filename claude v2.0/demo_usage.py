...
def find_verses_about_concept(concept_roots: List[str],
    ...
    Args:
        concept_roots: List of roots defining the concept (e.g., ['رحم', 'غفر'] for mercy)
...
def demo_mercy_search(composer, embedding_db, parser):
    ...
    # Define mercy concept by roots
    mercy_roots = [
        'رحم',  # mercy (الرحمن، الرحيم، رحمة)
        'غفر',  # forgiveness (الغفور، الغفار، مغفرة)
        'عفو',  # pardoning (العفو، عفا)
    ]
    ...
    for i, (verse_id, score) in enumerate(results, 1):
        ...
        verse_roots = set(ann.root for ann in annotations)
        
        has_mercy_roots = [r for r in mercy_roots if r in verse_roots]
        
        print(f"{i:2d}. {verse_id:8s} | score: {score:.3f} | roots: {has_mercy_roots if has_mercy_roots else 'none (related)'}")
